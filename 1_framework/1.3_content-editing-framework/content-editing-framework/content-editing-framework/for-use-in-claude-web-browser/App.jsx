...
// ── Shared components ─────────────────────────────────────────────────────────
// Shared questions checkbox — used identically across all three input tabs
function QCheck({ skipQ, setSkipQ }) {
  return (
...
function p0(tr) {
  return `Create 10 meaningful multiple-choice questions with 4 possible answers each from the following transcript.
...
function p1direct(tr) {
  return `Extract 10 main learning topics from the following transcript. For each topic, provide a clear title and 3–5 subtopics describing its key concepts.
...
function p3(topics) {
  ...
  return `Generate exactly 5 interactive React component ideas for each educational concept below, ranked by how well they achieve the learning objectives. Exactly 2 ideas must use animation; the other 3 must not.

${sections}

Composition rules:
- Exactly 2 animated ideas per topic (needsAnimation: true): animation must genuinely serve the concept (e.g. blood flow, signal propagation, cell division, pendulum). NOT decorative. These MUST include a speed slider, play/pause button, and reset button.
- Exactly 3 non-animated ideas per topic (needsAnimation: false): use rich hover/click interactions — hover to reveal labels, click to toggle states, side-by-side comparison panels, layered diagrams.
- All ideas: white background, color-coded zones, information revealed on demand, minimal always-visible text.
- Each description: 1-2 sentences — what the user sees and the key interaction or animation.

Return ONLY a valid JSON array, no other text:
[{"topicId":1,"topicTitle":"Name","ideas":[{"id":1,"icon":"🔄","title":"Title","description":"What it shows and how interaction or animation works.","needsAnimation":true},{"id":2,"icon":"🎨","title":"...","description":"...","needsAnimation":false},{"id":3,"icon":"🔍","title":"...","description":"...","needsAnimation":false},{"id":4,"icon":"🧩","title":"...","description":"...","needsAnimation":false},{"id":5,"icon":"🌊","title":"...","description":"...","needsAnimation":true}]}]`;
}

function pImpl(topic, ideaObj, customText) {
  const desc = customText?.trim() || `${ideaObj?.title||""}: ${ideaObj?.description||""}`;
  const animGuide = ideaObj?.needsAnimation === true
    ? `This component requires animation to teach the concept. Include smooth, purposeful animation that visualises the process. You MUST include a compact control bar with:
  1. A speed slider (label: "Speed") from 0.5× to 3×, adjusting animation speed in real time via a multiplier on the interval/timing
  2. A Play/Pause toggle button (▶ when paused, ⏸ when playing)
  3. A Reset button that stops the animation and returns everything to its initial state
  Place these as a small, clean row at the bottom or top — never dominating the visual.`
    : `Use rich static interactivity: hover to reveal labels and details, click to toggle between contrasting states, side-by-side comparison panels, layered diagrams. Do NOT add decorative or idle animations. Animations are forbidden unless the concept is literally about motion.`;
...
export default function App() {
  ...
  const [skipQ,       setSkipQ]      = useState(true);    // skip questions step (off by default)
  ...
  const genQuestions = () => {
    ...
    setLoading(true); setLoadMsg("Analyzing transcript and generating 10 questions\u2026");
    run(async () => {
      const raw    = await callClaude(p0(transcript), 5000, JSON_SYSTEM);
  ...
  const genTopicsFromImage = () => {
    ...
    run(async () => {
      ...
      const raw = await callClaudeVision(
        [{ base64, mimeType }], pImageTopics(), 3000, JSON_SYSTEM
      );
      const parsed = tryParseJSON(raw);
  ...
  const genTopicsDirect = () => {
    if (!transcript.trim()) return setError("Please enter or upload a transcript first.");
    setLoading(true); setLoadMsg("Extracting topics directly from transcript\u2026");
    run(async () => {
      const raw    = await callClaude(p1direct(transcript), 5000, JSON_SYSTEM);
      const parsed = tryParseJSON(raw);
  ...
  const genTopics = () => {
    setLoading(true); setLoadMsg("Extracting 10 topics from your questions\u2026");
    run(async () => {
      const raw    = await callClaude(p1(questions), 5000, JSON_SYSTEM);
      const parsed = tryParseJSON(raw);
      if (!Array.isArray(parsed)||!parsed.length) throw new Error("Couldn\u2019t parse topics \u2014 try again.");
      setTopics(parsed); setStep(3);
    });
  ...
  return (
    <div style={{minHeight:"100vh",background:T.surface,fontFamily:"'Inter','Segoe UI',system-ui,sans-serif"}}>
      ...
      <div style={{width:"96vw",maxWidth:1800,margin:"0 auto",padding:"28px clamp(16px, 4vw, 48px) 80px"}}>
            ...
            <div style={{background:"#fff",border:`1px solid ${T.border}`,borderRadius:14,padding:24}}>
              ...
              {inputMode==="transcript"&&(
                <div>
                  ...
                  <QCheck skipQ={skipQ} setSkipQ={setSkipQ}/>
                              ...
                              <div key={idea.id}
                                ...
                                {isEditing?(
                                  ...
                                  <>
                                    ...
                                    {idea.needsAnimation!==undefined&&(
                                      <span style={{marginTop:6,display:"inline-block",fontSize:10,fontWeight:700,
                                        background:idea.needsAnimation?"#fff7ed":"#eff6ff",
                                        color:idea.needsAnimation?"#c2410c":T.blue,
                                        border:`1px solid ${idea.needsAnimation?"#fed7aa":T.blueBorder}`,
                                        borderRadius:10,padding:"1px 7px"}}>
                                        {idea.needsAnimation?"🎬 animated":"🖱 interactive"}
                                      </span>
                                    )}
                                  </>
                                )}
